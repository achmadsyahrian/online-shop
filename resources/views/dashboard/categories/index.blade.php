                    ...
                    <tbody>
                        ...
                        @endphp
                            <tr>
                                ...
                                <td class="border-bottom-0">
                                    <a href="/dashboard/categories/{{ $category->id }}/edit" class="badge bg-warning" style="text-decoration: none !important; color: white !important;">
                                        <i class="ti ti-edit"></i>
                                    </a>
                                    <!-- Perbarui elemen form dengan id yang unik untuk setiap kategori -->
                                    <form action="/dashboard/categories/{{ $category->id }}" method="POST" class="d-inline" id="formDelete_{{ $category->id }}">
                                      @method('delete')
                                      @csrf
                                      <button type="submit" onclick="event.preventDefault(); deleteData('{{ $category->id }}');" class="badge bg-danger border-0">
